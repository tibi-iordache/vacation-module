...
class ProjectResourceIT {
    ...
    void getAllProjectsByProjectManagerIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, User.class).isEmpty()) {
            projectRepository.saveAndFlush(project);
            projectManager = UserResourceIT.createEntity(em);
        } else {
            projectManager = TestUtil.findAll(em, User.class).get(0);
        }
        ...
        // Get all the projectList where projectManager equals to projectManagerId
        defaultProjectShouldBeFound("projectManagerId.equals=" + projectManagerId);

        // Get all the projectList where projectManager equals to (projectManagerId + 1)
        defaultProjectShouldNotBeFound("projectManagerId.equals=" + (projectManagerId + 1));
    }

    @Test
    @Transactional
    void getAllProjectsByTechLeadIsEqualToSomething() throws Exception {
        User techLead;
        if (TestUtil.findAll(em, User.class).isEmpty()) {
            projectRepository.saveAndFlush(project);
            techLead = UserResourceIT.createEntity(em);
        } else {
            techLead = TestUtil.findAll(em, User.class).get(0);
        }
        em.persist(techLead);
        em.flush();
        project.setTechLead(techLead);
        projectRepository.saveAndFlush(project);
        Long techLeadId = techLead.getId();

        // Get all the projectList where techLead equals to techLeadId
        defaultProjectShouldBeFound("techLeadId.equals=" + techLeadId);

        // Get all the projectList where techLead equals to (techLeadId + 1)
        defaultProjectShouldNotBeFound("techLeadId.equals=" + (techLeadId + 1));
    }
